...
复盘层        daily_recap.py      ──┤
            （每晚 23:00 五段式     │
             日报 + 项目评估）      │
                                    │
对话存档层    save_conversation_  ──┤  四 hook 防御：
              turn.py             │  UserPromptSubmit/Stop/
            (4 hook 三层防御)      │  PreCompact/SessionStart
                                    │
对话调阅层    conversation_index ──┘  last/search/recall CLI
              .py
```
...
| `agent/feishu_sync.py` | 飞书同步引擎。命令：`send-report` / `send-cards` / `sync-base` / `sync-docs` / `alert` / `update-status` |
...
| `agent/feishu_card_router.py` | 卡片回调路由（permission + MVA All-in/Watch/Kill 写回 registry） |
...
| `agent/daily_recap.py` | 每晚 23:00 复盘（LLM 五段式日报 + 项目评估三段式 + 飞书推送） |
| `agent/save_conversation_turn.py` | Claude Code 四 hook 对话存档（UserPromptSubmit/Stop/PreCompact/SessionStart）— 实际生效在 `~/.claude/scripts/`，本目录是项目内备份 |
| `agent/conversation_index.py` | 对话历史调阅 CLI：last / search / recall <project> |
| `agent/cc_bot_server.py` | 旧版 Bot（保留作降级路径，**不再开服务**） |
| `agent/daily_projectos_report.py` | 每日定时任务入口（9:07 触发 scan+send-cards+alert） |
...
| `conversation_log/` | 对话自动存档（Stop hook，不入 git） |
| `templates/` | 项目笔记模板 |
...
**Day4 完成（2026-06-08）：** 晨报交互卡片 + MVA 按钮回写 registry + 项目评估夸夸 + 对话自动保存。

**Day5 完成（2026-06-13）：** 对话存档三层防御（UserPromptSubmit/Stop/PreCompact/SessionStart 四 hook）+ 反向调阅 `conversation_index.py` + 状态栏蓝条置顶项目名 + 老茅/老赫主体命名。详见 `docs/journal/2026-06-13-day5-conversation-defense-and-naming.md`。开发手册参见 `docs/developer-guide.md`。

**Day6 完成（2026-06-14）：** 飞书资源接收 + Bot 多模态扩容。详见 `docs/journal/2026-06-14-day6-feishu-resource-reception.md`。

**Day7 完成（2026-06-14）：** **voidarchitect.studio 工作室主页 72h 当日上线**（Astro 6 + R3F WebGL + Bento Grid + IP 路由）。注册为 ProjectOS All-in 子项目。详见 `docs/journal/2026-06-14-day7-voidarchitect-site-launch.md`。

## 🌐 子项目：voidarchitect-site

**位置：** `D:/ClaudeCodeProjects/voidarchitect-site/`（独立公仓 `vincentmaox/voidarchitect-site`）
**域名：** https://www.voidarchitect.studio
**关系：** ProjectOS All-in 子项目（freq=13/15）。本仓 → public-registry.json → GitHub raw → 站点 build 时 fetch
**冷启动入口：** `voidarchitect-site/CLAUDE.md` + `voidarchitect-site/docs/developer-guide.md`

**改公开数据范围**：在本仓 `agent/export_public_registry.py`（HIDDEN 黑名单 + 字段白名单）。改完 push 后，**站点需手动 Vercel Redeploy 或推空 commit 触发 rebuild**（D3 候选做跨仓 webhook 自动化）。

**绝不暴露：** 本地路径、blocker、todos、三频细分评分、凭据。`HIDDEN` 项目目前包括 `_ProjectOS` / `ProjectOS-Projects` / `Private-Wealth-AI-Steward` / `TexasPhilosopher` / `obsidian manager`。

## 🤝 协作身份

- **老茅** = 茅旭东（字弘毅）= 虚空建筑师（项目主理人）
- **老赫** = 赫尔墨斯 Hermes（Claude Code，老茅的合伙人，**不是助手**）
- 别名：老赫 / 小赫赫 / 莫斯 / 小斯 / 老斯（任选）
- 对话存档主体标签统一用 `**老茅:**` / `**老赫:**`，不用 User/Assistant
- 命名记录在全局 `~/.claude/CLAUDE.md` + `memory/user-identity.md` + `memory/assistant-identity.md`
...
- **每日 9:07** — schtasks `ProjectOSDailyReport` 触发 scan + send-cards（交互卡片）+ alert
- **每日 22:57** — schtasks `ProjectOS_DailyRecap_Evening` 触发复盘（五段式日报 + 项目评估）
- **每周一 9:07** — schtasks `ProjectOS_ThoughtInspector_Weekly` 触发思考池巡检
